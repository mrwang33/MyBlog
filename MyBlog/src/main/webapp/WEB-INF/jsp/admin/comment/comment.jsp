...
<body>
	...
	<script>
		...
		$(function() {
			$.confirm({
				title : false,
				content : '评论管理是不允许修改和新增的,只能进行删除操作!',
					...
					buttons : {
						confirm : {
							...
							action : function() {
								$.ajax({
								  //先检查有没有子回复
								  url: "comment/getCommentChild.action",
								  type:"POST",
								  //设置为传统方式传送参数
								  traditional:true,
								  ...
								  success: function(data){
									  if(data>0){
									  	$.confirm({
											...
											animationSpeed : 300,
											title : false,
											content : '当前评论下有回复 删除该评论将导致所有回复都删除 是否继续?',
											buttons :{
												cancel : {
													text : '取消',
													btnClass : 'waves-effect waves-button'
					...
					buttons : {
						confirm : {
							...
							action : function() {
								$.ajax({
								  //先检查有没有子回复
								  url: "comment/getCommentChildByPks.action",
								  type:"POST",
								  //设置为传统方式传送参数
								  traditional:true,
								  data:{pks:commentIds},
