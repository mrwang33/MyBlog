...
<script>
    $(document).ready(function(){
        ...
        $.getScript(cityUrl, function(script, textStatus, jqXHR) {
            ...
            var url = "/weather.action?loc=" + citytq;
            $.ajax({
                url : url,
                success : function(data) {
                    var json = eval('(' + data + ')');
                    var weather = json.results[0];
					var _f = weather.now.code;
					var temperature = weather.now.temperature;
                    var img = "<img width='16px' height='16px' src='images/weather/" +_f
                        + ".png' />";
                    var tq =  "实时天气: "+citytq+" " +weather.now.text+""+img + " " +temperature+"℃";
