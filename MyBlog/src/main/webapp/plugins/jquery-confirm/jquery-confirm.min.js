...
(function($) {
	...
	$.dialog = function(options, option2) {
		...
		options.confirmKeys = [ 13 ];return jconfirm(options);
	};
	jconfirm = function(options) {
		...
		jconfirm.instances.push(instance);return instance;
	};
	Jconfirm = function(options) {
	...
	Jconfirm.prototype = {
		_init : function() {
			...
			this._id = Math.round(Math.random() * 99999);setTimeout(function() {
				that.open();
			}, 0);
		},
		_buildHTML : function() {
			...
			if (this.useBootstrap) {
				template.find(".jc-bs3-row").addClass(this.bootstrapClasses.row);template.find(".jconfirm-box-container").addClass(this.columnClassParsed);
			...
			template.find(".jconfirm-box").attr("aria-labelledby", ariaLabel).attr("tabindex", -1);template.find(".jconfirm-content").attr("id", ariaLabel);
			...
			this._modalReady = $.Deferred();this.setTitle();this.setIcon();this._setButtons();this._parseContent();
			if (this.isAjax) {
			...
			$.when(this._contentReady, this._modalReady).then(function() {
				if (that.isAjaxLoading) {
					...
					that.setContent();that.setTitle();that.setIcon();
					if (typeof that.onContentReady == "function") {
						that.onContentReady();
					}
				}
			...
			that._contentHeight = this.$content.height();this._watchContent();this.setDialogCenter();
		...
		setTheme : function(theme) {
			...
			this.theme = theme || this.theme;this._parseTheme(this.theme);
			if (previous) {
		...
		_parseTheme : function(theme) {
			var that = this;
			theme = theme.split(",");$.each(theme, function(k, a) {
		...
		_watchContent : function() {
			...
			this._timer = setInterval(function() {
				...
				if (that._contentHash != now || that._contentHeight != nowHeight) {
					...
					that._contentHeight = nowHeight;that.setDialogCenter();that._imagesLoaded();
				}
			}, this.watchInterval);
		},
		...
		_hiLightModal : function() {
			...
			var duration = 2;
			this._hilightAnimating = true;setTimeout(function() {
				that._hilightAnimating = false;that.$body.removeClass("hilight");
			}, duration * 1000);
		},
		_bindEvents : function() {
			var that = this;
			this.boxClicked = false;this.$scrollPane.click(function(e) {
				if (!that.boxClicked) {
		...
		_setButtons : function() {
			...
			$.each(this.buttons, function(key, button) {
				...
				that.buttons[key].keys = button.keys || [];$.each(that.buttons[key].keys, function(i, a) {
				...
				var button_element = $('<button type="button" class="btn ' + that.buttons[key].btnClass + '">' + that.buttons[key].text + "</button>").click(function(e) {
					...
					that.onAction(key);that._stopCountDown();
		...
		showLoading : function(disableButtons) {
			this.loadingSpinner = true;this.$jconfirmBox.addClass("loading");
			if (disableButtons) {
		...
		hideLoading : function(enableButtons) {
			this.loadingSpinner = false;this.$jconfirmBox.removeClass("loading");
			if (enableButtons) {
			...
			if (typeof this.content == "function") {
				...
				if (typeof res == "string") {
					...
					if (typeof res == "object" && typeof res.always == "function") {
						this.isAjax = true;
						this.isAjaxLoading = true;res.always(function(data, status, xhr) {
							that.ajaxResponse = {
								data : data,
								status : status,
								xhr : xhr
		...
		setDialogCenter : function() {
			...
			}).scrollTop(0);this.$body.css(style);
		},
		...
		close : function() {
			...
			this._unwatchContent();clearInterval(this.imageLoadInterval);$(window).unbind("resize." + this._id);$(window).unbind("keyup." + this._id);$("body").removeClass("jconfirm-no-scroll-" + this._id);this.$body.addClass(this.closeAnimationParsed);this.$jconfirmBg.addClass("jconfirm-bg-h");
			var closeTimer = (this.closeAnimation == "none") ? 1 : this.animationSpeed;
			setTimeout(function() {
				that.$el.remove();console.log(that._lastFocused);
		...
		_open : function() {
			...
			this.$body.removeClass(this.animationParsed);this.$jconfirmBg.removeClass("jconfirm-bg-h");this.$body.focus();setTimeout(function() {
				that.$body.css(that._getCSS(that.animationSpeed, 1));that.$body.css({
